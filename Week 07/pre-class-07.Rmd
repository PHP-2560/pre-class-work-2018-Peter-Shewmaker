...
```{r}
#Same process, except now the starting probability is 0.48 + q, where q is a uniform random variable from -0.02 to 0.02 (the new value will between 2% either way). Instead of 100000 hands we will do 10000 to save time. Then we will find the value of q for which the game is most fair, testing 100 values of q.

end_bankroll_means <- matrix(nrow = 2, ncol = 100)
for(j in 1:100){
  starting_p <- 0.48 + runif(1, -0.02, 0.02)
  p <- starting_p
  end_bankroll_10000 <- c()
  bankroll = 0
  for(i in 1:10000){
    if(p >= 1){
      p = 1
      ...
      end_bankroll_10000[i] <- bankroll
  }
  end_bankroll_means[,j] <- c(starting_p,mean(end_bankroll_10000))
}

end_bankroll_means[2,] <- abs(end_bankroll_means[2,])
a <- which(end_bankroll_means[2,] == min(end_bankroll_means[2,]))
fairest_p <- end_bankroll_means[,a][1]
#This returns the fairest probability value as well as the absolute value of the mean of the bankroll after the 10000 hands.
fairest_p
end_bankroll_means[,a][2]
```
